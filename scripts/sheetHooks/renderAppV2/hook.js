...
import {
  installFatigueAttributeMarking,
  installUseScarButtons,
  installChooseAttributeButtons,
  installUseDirectiveButton,
  installSupportingImprovementButtons,
  installSupportingMilestoneButton,
} from "./sheetButtons.js";
...
function installOfficersLogButtonsInStaTracker(app, root) {
  try {
    ...
    // --- Mission Directives Section (visible to all players) ---
    installMissionDirectivesInStaTracker(root, row);

    // GM-only: add Officers Log buttons next to the roll buttons.
...
export function installRenderApplicationV2Hook() {
  ...
  Hooks.on("renderApplicationV2", (app, root /* HTMLElement */, _context) => {
    ...
    // Check if this is a STA character or supporting character sheet
    const isMainCharSheet = app?.id?.startsWith?.("STACharacterSheet2e");
    const isSupportingSheet = app?.id?.startsWith?.("STASupportingSheet2e");

    // Supporting characters: only process Use Value buttons, skip everything else
    if (isSupportingSheet) {
      const actor = app.actor;
      if (!actor || actor.type !== "character") return;

      // Add per-Value "Use Value" button for supporting characters
      ...
      for (const entry of valueEntries) {
        ...
        const onUseSupportingChar = async (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
          ...
          if (valueIsTrauma && choice === "challenge") {
            ...
            // Supporting characters: no log updates, no callbacks
            app.render();
            return;
          }

          // GM can apply effects directly
          if (game.user.isGM) {
          ...
          // Players: positive use applies directly
          if (valueState === "positive") {
            await spendDetermination(actor);
            if (valueIsTrauma) {
              await adjustStress(1); // Trauma positive: +1 stress
          ...
          // Players: GM approval required for negative and challenge
          ...
          try {
            const result = await moduleSocket.executeAsGM(
              "requestValueUseApproval",
              {
                ...
                valueName: valueItem.name,
                usage: choice,
                currentMissionLogId: null, // Supporting characters have no mission logs
                isTrauma: valueIsTrauma,
                isSupportingCharacter: true, // Flag to skip log/callback handling
              },
            );
        ...
        if (!challenged) {
          useBtn.addEventListener("click", onUseSupportingChar);
          useBtn.addEventListener("keydown", (ev) => {
            if (ev.key === "Enter" || ev.key === " ") onUseSupportingChar(ev);
          });
        }
      ...
      // Supporting characters: install fatigue attribute marking
      try {
        installFatigueAttributeMarking(root, actor);
      } catch (_) {
        // ignore
      }

      // Supporting characters: install "Use Scar" buttons
      try {
        installUseScarButtons(root, actor, {
          isSupportingCharacter: true,
      ...
      // Supporting characters: install "Choose Attribute" buttons for fatigue traits
      try {
        installChooseAttributeButtons(root, actor, {
          onComplete: () => app.render(),
        });
      } catch (_) {
        // ignore
      ...
      // Supporting characters: install "Use Directive" button
      try {
        installUseDirectiveButton(root, actor, {
          isSupportingCharacter: true,
          onComplete: () => app.render(),
        });
      } catch (_) {
      ...
      // Supporting characters: install improvement buttons (replacing checkbox grid)
      try {
        installSupportingImprovementButtons(root, actor, {
          onComplete: () => app.render(),
        });
      } catch (_) {
        // ignore
      ...
      // Supporting characters: install milestone button (opens benefit picker dialog)
      try {
        installSupportingMilestoneButton(root, actor);
      } catch (_) {
        // ignore
      }

      // Supporting characters: done processing, no further character sheet features
      return;
    }

    // Only target STA main character sheet app (not supporting characters)
    if (!isMainCharSheet) return;
    ...
    try {
      installCharacterLogListResizer(root, actor);
    } catch (_) {
      // ignore
    }
